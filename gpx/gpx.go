...
func (rte *GPXRoute) Length() float64 {
	// TODO: npe check
	points := make([]Location, len(rte.Points))
	for pointNo, point := range rte.Points {
		points[pointNo] = point
	}
...
func (seg *GPXTrackSegment) Length2D() float64 {
	...
	points := make([]Location, len(seg.Points))
